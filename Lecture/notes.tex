...
\end{algorithm}
We now justify the Pseudo-marginal Monte Carlo algorithm by showing that it is actually a generalized MH algorithm (as described in \Lemref{generalMH}) by considering extended Markov chain, $(\bar X_k)_{k\in\nset}=(X_k,W_k)_{k\in\nset}$ on an extended space and with an extended target. Define the extended target distribution $\bar{\pi}(\rmd \bar x)=\bar{\pi}(\rmd x\rmd w)=w R(x,\rmd w) \mu(\rmd x)$ (where we set $\bar x=(x,w)$). Note that $\bar{\pi}$ is indeed a probability measure on $\bar\Xset=\Xset \times \rset^+_\star$, since
$$
\int_{\Xset\times\rset^+_\star} \bar{\pi}(\rmd x\rmd w)=\int_\Xset \lr{ \int_{\rset^+_\star} w R(x,\rmd w)}\mu(\rmd x)=\int_\Xset \pi(x)\mu(\rmd x)=1\eqsp.
$$
...
$$
Since $r$ is positive, we can apply \Lemref{generalMH} with $\alpha(\bar x,\bar x')=r(\bar x,\bar x') \wedge 1$ and we finally get that $\mh{\bar{\pi},\bar Q}(\bar x,\rmd \bar x')$ is $\bar{\pi}$-reversible. Since Algorithm~\ref{alg:pseudo:mh} corresponds to applying the Markov kernel $\mh{\bar{\pi},\bar Q}$, this completes the proof. Note that the extended target distribution $\Pi$ has the marginal $\pi$ with respect to the first component:
$$
\bar{\pi}(A \times \rset^+_\star)=\int_A \int_{\rset^+_\star} w R(x,\rmd w) \mu(\rmd x)=\int_A \pi(\rmd x)=\pi(A).
$$
To sum up, $(\bar X_k)_{k\in\nset}=(X_k,W_k)_{k\in\nset}$ produced by Algorithm~\ref{alg:pseudo:mh} is a generalized Metropolis-Hastings algorithm where the target distribution $\bar{\pi}$ admits $\pi$ as the marginal distribution on the first component.
...
\[
\widehat S^{(k)}(\theta^{(k)}) := \frac{1}{M_k}\sum_{m=1}^{M_k} S\!\left(X_{k,m},Y\right).
\]
...
\phi(\theta) + \left\langle \widehat S^{(k)}(\theta^{(k)}),\ \psi(\theta)\right\rangle
\right\}.
\]
...
a Markov chain Monte Carlo (MCMC) method targeting $p_{\theta^{(k)}}(X\mid Y)$. Specifically, at iteration $k$, let $\{X_{k,t}\}_{p\ge 1}$ be a Markov chain with
...
\[
\widehat S^{(k)}_{\mathrm{MCMC}}(\theta^{(k)})
:= \frac{1}{M_k}\sum_{p=B_k+1}^{B_k+M_k} S\!\left(X_{k,p},Y\right),
\]
which replaces $\bar S(\theta^{(k)})$ in the E-step. The resulting MCMC-EM update is
\[
\theta^{(k+1)} \in \arg\max_{\theta\in\Theta}\left\{
\phi(\theta) + \left\langle \widehat S^{(k)}_{\mathrm{MCMC}}(\theta^{(k)}),\ \psi(\theta)\right\rangle
\right\}.
\]
...
\begin{example}
Assume that we observe independent data $Y = (Y_1,\ldots,Y_n)$
...
and
\[
\lambda_i = \exp(\beta + X_i)\eqsp,
\]
with independent latent variables $X=(X_1,\ldots,X_n)$ such that for all $1\leq i \leq n$,
\[
X_i \sim \mathcal{N}(0,\sigma^2)\eqsp.
\]
The unknown parameter is $\theta = (\beta,\sigma^2)$. The complete-data likelihood is given by
\[
p_\theta(X,Y)
=
\prod_{i=1}^n
p_\theta(Y_i|X_i)\,p_\theta(X_i)\eqsp.
\]
The conditional distribution of the observations is
\[
p_\theta(Y_i|X_i)
=
\frac{\exp(-\exp(\beta+X_i))\exp(Y_i(\beta+X_i))}{Y_i!}
\]
and the prior distribution of the latent variable is
\[
p_\theta(X_i)
=
(2\pi\sigma^2)^{-1/2}
\exp\left(-\frac{X_i^2}{2\sigma^2}\right)\eqsp.
\]
Therefore, the complete-data loglikelihood is 
\[
\log p_\theta(X,Y) = \sum_{i=1}^n \Big[ Y_i(\beta+X_i)-\exp(\beta+X_i)\Big]-\sum_{i=1}^n\frac{X_i^2}{2\sigma^2}
-
\frac{n}{2}\log\sigma^2 + C\eqsp,
\]
where $C$ is a constant that does not depend on $\theta$. Then, at iteration $k\geq 0$ of the algorithm, the intermediate quantity of the EM algorithm is 
\[
Q(\theta,\theta^{(k)})
=
\sum_{i=1}^n \mathbb{E}_{\theta^{(k)}}
\left[
  Y_i(\beta+X_i)-\exp(\beta+X_i)- \frac{X_i^2}{2\sigma^2}
 \middle| Y_i
\right]  -
\frac{n}{2}\log\sigma^2 + C\eqsp.
\]
However the conditional distribution $p_{\theta^{(k)}}(X_i|Y_i)$ does not have a closed form. However, we can write
\[
p_{\theta^{(k)}}(X_i|Y_i)
\propto
p_{\theta^{(k)}}(Y_i|X_i)p_{\theta^{(k)}}(X_i)\eqsp.
\]
Therefore, the intermediate quantity  cannot be computed analytically. 
Since direct sampling from \(p_{\theta^{(k)}}(X_i|Y_i)\) is not possible but this distribution is known up to a multiplicative constant, we may use any MCMC method  (Random Walk Metropolis Hastings, MALA, HMC) to approximate the intermediate quantity. Let $\{X^{(k)}_{p}\}_{p\geq 0}$ be the Markov chain generated by the chosen MCMC algorithm with target distribution $p_{\theta^{(k)}}(X|Y)$.  The parameters are updated by maximizing the Monte Carlo approximation
\[
\theta^{(k+1)}
=
\arg\max_{\theta}
\frac{1}{M_k}
\sum_{p=B_k+1}^{B_k+M_k}
\log p_\theta(X^{(k)}_{p},Y)\eqsp.
\]
%and 
%\[
%\beta^{(k+1)}
...
%\Big]\eqsp.
%\]
%This maximization can be performed numerically.
%
%
%\[
%\sigma^{2(t+1)}
...
%\begin{enumerate}
%\item Run an MCMC chain targeting $p(X|Y,\theta^{(t)})$.
%\item Obtain samples $X^{(1)},\ldots,X^{(M)}$.
%\item Approximate
%\[
%Q(\theta|\theta^{(t)})
%\approx
...
In variational inference, we introduce a variational family i.e. a family of densities to  approximate $z\mapsto p(z|x)$. Let $\mathcal{D}$ be such a family, where the densities $q(\cdot|x) \in\mathcal{D}$ satisfy the two following assumptions.
\begin{itemize}
\item For all $q(\cdot|x)\in \mathcal{D}$, $z\mapsto q(z|x)$ is easy to evaluate.
\item For all $q(\cdot|x)\in \mathcal{D}$, $Z\sim q(\cdot|x)$ is easy to sample from.
\end{itemize}

\medskip

Then, for all $x$ and all $q(\cdot|x)\in \mathcal{D}$, writing $\mathrm{KL}$ the Kullback-Leibler divergence between two probability distributions,
\begin{align*}
\mathrm{KL}\left(q(\cdot|x)\|p(\cdot|x)\right) = \int q(z|x) \log \frac{q(z|x)}{p(z|x)} \mathrm{d}z&= \mathbb{E}_{q(\cdot|x)}[\log q(Z|x)] - \mathbb{E}_{q(\cdot|x)}[\log p(Z|x)]\,,\\
 &= \mathbb{E}_{q(\cdot|x)}[\log q(Z|x)] - \mathbb{E}_{q(\cdot|x)}[\log p(Z,x)]+\log p(x)\,,\\
&= -\mathcal{L}(q(\cdot|x))+\log p(x)\,,
\end{align*}
where
$$
\mathcal{L}(q(\cdot|x)) = \mathbb{E}_{q(\cdot|x)}\left[\log \frac{p(Z,x)}{q(Z|x)}\right]\,.
$$
Using Jensen's inequality, we obtain $\mathrm{KL}\left(q(\cdot|x)\|p(\cdot|x)\right) \geq 0$ so that
$$
\mathcal{L}(q(\cdot|x))\leq \log p(x)\,.
$$
This inequality justifies the name Evidence Lower BOund for $\mathcal{L}(q(\cdot|x)) = \mathbb{E}_{q(\cdot|x)}[\log (p(Z,x)/q(Z|x))]$. In variational inference, we then aim to approximate  $p(\cdot|x)$ by $q_{*}(\cdot|x)$ where:
$$
q_*(\cdot|x) \in \mathrm{argmax}_{q\in \mathcal{D}}\;\mathcal{L}(q(\cdot|x))\,.
$$
...
$$
\mathcal{D} = \left\{z\mapsto q(z|x) = \prod_{j=1}^dq_j(z_j|x)\;;\; q_j(\cdot|x) \,\mbox{is\, a\, density} \right\}\,.
$$
Even when considering a simple variational family such as $\mathcal{D}$, it is not possible to maximize the ELBO explicitly. Assume therefore that we want to optimize $\mathrm{ELBO}(q(\cdot|x)) $ on $q_j(\cdot|x)$ only for some $1\leq j \leq d$, the other densities $(q_\ell(\cdot|x))_{\ell\neq j}$ being kept fixed.
Write $z_{-j} = (z_\ell)_{1\leq \ell \leq d; \ell \neq j}$, and for all $q(\cdot|x)\in \mathcal{D}$,  $q_{-j}(z_{-j}|x) = \prod_{1\leq \ell \leq d; \ell \neq j}q_{\ell}(z_{\ell}|x)$. Then,
\begin{align*}
\mathcal{L}(q(\cdot|x)) &= \int \left\{\prod_{\ell=1}^dq_\ell(z_\ell|x)\right\} \left\{\log (p(z_{-j},x)p(z_j|z_{-j},x)) - \sum_{\ell=1}^d\log q_\ell(z_\ell|x)\right\}\rmd z_1\ldots \rmd z_d\,,\\
&= \int q_j(z_j|x)\int \left\{\prod_{\ell=1, \ell\neq j}^dq_\ell(z_\ell|x)\right\} \left\{\log (p(z_{-j},x)p(z_j|z_{-j},x))\rmd z_{-j}\right\}\rmd z_j \\
&\hspace{3cm}- \sum_{\ell=1}^d\int q_\ell(z_\ell|x)\log q_\ell(z_\ell|x)\rmd z_\ell  \,\\
&= \mathbb{E}_{q_j(\cdot|x)}\left[\mathbb{E}_{q_{-j}(\cdot|x)}\left[\log p(Z_j | Z_{-j},x)\right]\right] - \mathbb{E}_{q_j(\cdot|x)}\left[\log q_j(Z_j|x)\right]  + \mathrm{C}
\end{align*}
%\begin{align*}
...
%\end{align*}
where $\mathrm{C}$ does not depend on $q_j(\cdot|x)$. %,  $z_{-j} = (z_\ell)_{1\leq \ell \leq d; \ell \neq j}$ and $q_{-j}(z_{-j}) = \prod_{1\leq \ell \leq d; \ell \neq j}q_{\ell}(z_{\ell})$.  
Consider the density $\tilde q_j(\cdot|x)$ such that
$$
\tilde q_j(z_j|x) \propto \exp\left(\mathbb{E}_{q_{-j}(\cdot|x)}\left[\log p(z_j | Z_{-j},x)\right]\right)\,,
$$
i.e. the density given by $\tilde q_j(z_j|x)= \exp(\mathbb{E}_{q_{-j}(\cdot|x)}[\log p(z_j | Z_{-j},x)])/C_j$ where $C_j$ does not depend on $z_j$ ($C_j$ is the normalizing constant to obtain a density). Therefore,
$$
\mathcal{L}(q(\cdot|x)) = -\mathbb{E}_{q_j(\cdot|x)}\left[\log \frac{q_j(Z_j)}{\tilde q_j(Z_j|x) }\right]  + \mathrm{C} + \log C_j = -\mathrm{KL}(q_j(\cdot|x)\|\tilde q_j(\cdot|x))  + \mathrm{C} + \log C_j\,.
$$
Therefore, optimizing  $\mathcal{L}(q(\cdot|x)) $ on $q_j(\cdot|x)$ only, the other densities $(q_\ell(\cdot|x))_{\ell\neq j}$ being kept fixed, yields an optimum given by $\tilde q_j(\cdot|x)$.
The algorithm Coordinate Ascent Variational Inference (CAVI) proposes therefore to sequentially update  $q_j(\cdot|x)$, $1\leq j \leq d$ until a stopping criterion is met. In Algorithm~\ref{alg:CAVI}, we propose a version of the algorithm where the variational distribution of only one component of $Z$ is updated at each iteration, of course many alternatives can be considered. A standard alternative is to update each variational distribution at each iteration.

\begin{algorithm}[H] \label{alg:CAVI}
    \KwData{Observation $x$, initial variational distribution $\{q_j^{(0)}(\cdot|x)\}_{1\leq j \leq d}$, maximum number of iteration $N$}
    \KwResult{A variational distribution for each coordinate of $Z$, $q^{(N)}_j(\cdot|x)$, $1\leq j\leq d$.}
    \For {$ k= 1 \to N$}{
        Draw $j\in\{1,\ldots,d\}$ uniformly at random;

        Set $q^{(k)}_\ell(\cdot|x) = q^{(k-1)}_\ell(\cdot|x)$ for all $1\leq \ell \leq d$, $\ell \neq j$ and $q^{(k)}_{-j}(\cdot|x) = \prod_{1\leq \ell \leq d, \ell\neq j} q^{(k)}_{\ell}(\cdot|x)$;

        Set $q^{(k)}_j(z_j|x) \propto \exp\left(\mathbb{E}_{q_{-j}^{(k)}(\cdot|x)}\left[\log p(z_j | Z_{-j},x)\right]\right)$;
...
$$
Writing $z = (\mu,c)$ and $x = x_{1:n}$, our objective is to estimate $p(\mu,c|x)$ where $c = (c_1,\cdots,c_n)$ are the components of the observations. Consider the following initialization of the CAVI algorithm:
$$
q(\mu,c|x) = \prod_{k=1}^K \varphi_{m_k,s_k}(\mu_k)\prod_{i=1}^n \mathrm{Cat}_{\phi_i}(c_i)\,, 
$$
which means that under $q(\cdot|x)$:
\begin{itemize}
...
Note that
\begin{align*}
\mathrm{KL}\left(q(\cdot|x)\|p(\cdot|x)\right) &= \mathbb{E}_{q(\cdot|x)}[\log q(\mu,c|x)] - \mathbb{E}_{q(\cdot|x)}[\log p(\mu,c|x)]\,,\\
 &= \mathbb{E}_{q(\cdot|x)}[\log q(\mu,c|x)] - \mathbb{E}_{q(\cdot|x)}[\log p(\mu,c,x)]+\log p(x)\,,\\
&= -\mathcal{L}(q(\cdot|x))+\log p(x)\,,
\end{align*}
where
$$
\mathcal{L}(q(\cdot|x))= -\mathbb{E}_{q(\cdot|x)}[\log q(\mu,c|x)] + \mathbb{E}_{q(\cdot|x)}[\log p(\mu,c,x)] \,.
$$
CAVI algorithm computes iteratively $1\leqslant k \leqslant K$,
$$
q(\mu_k|x) \propto \mathrm{exp}\left(\mathbb{E}_{\tilde q_{\mu_k}(\cdot|x)}[\log p(\mu_k|x,c,\mu_{-k})]\right)
$$
and for all $1\leqslant i \leqslant n$,
$$
q(c_i|x) \propto \mathrm{exp}\left(\mathbb{E}_{\tilde q_{c_i}(\cdot|x)}[\log p(c_i|x,c_{-i},\mu)]\right)\,,
$$
where $\mu_{-k} = (\mu_j)_{1\leq j \leq K, j\neq k}$, $c_{-i} = (c_j)_{1\leq j \leq n, j\neq i}$, and  $\mathbb{E}_{\tilde q_z(\cdot|x)}$ is the expectation under the variational distribution of all variables except $z$. 
%\begin{itemize}
...
Then,
$$
\mathbb{E}_{\tilde q_{c_i}(\cdot|x)}[\log p(c_i|x,c_{-i},\mu)] = \log p(c_i) + \sum_{k=1}^K 1_{c_i=k} \mathbb{E}_{\tilde q_{c_i}(\cdot|x)}[\log \varphi_{\mu_k,1}(x_i)]
$$
and
\begin{align*}
\mathrm{exp}\left(\mathbb{E}_{\tilde q_{c_i}(\cdot|x)}[\log p(c_i|x,c_{-i},\mu)]\right) &\propto p(c_i) \mathrm{exp}\left(\sum_{k=1}^K 1_{c_i=k} \mathbb{E}_{\tilde q_{c_i}(\cdot|x)}[\log \varphi_{\mu_k,1}(x_i)]\right)\,\\
&\propto p(c_i) \mathrm{exp}\left(\sum_{k=1}^K 1_{c_i=k} \mathbb{E}_{\tilde q_{c_i}(\cdot|x)}[-(x_i-\mu_k)^2/2]\right)\,.
\end{align*}
The update writes:
$$
...
Then,
$$
\mathbb{E}_{\tilde q_{\mu_k}(\cdot|x)}[\log p(\mu_k|x,c,\mu_{-k})] = \log p(\mu_k) + \sum_{i=1}^n \mathbb{E}_{\tilde q_{\mu_k}(\cdot|x)}[\log p(x_i|\mu,c_i)]
$$
and
\begin{align*}
\mathrm{exp}\left(\mathbb{E}_{\tilde q_{\mu_k}(\cdot|x)}[\log p(\mu_k|x,c,\mu_{-k})]\right) &\propto p(\mu_k) \mathrm{exp}\left(\sum_{i=1}^n\sum_{\ell=1}^K  \mathbb{E}_{\tilde q_{\mu_k}(\cdot|x)}[1_{c_i=\ell}\log \varphi_{\mu_\ell,1}(x_i)]\right)\,\\
&\propto p(\mu_k) \mathrm{exp}\left(\sum_{i=1}^n \phi_i(k) \mathbb{E}_{\tilde q_{\mu_k}(\cdot|x)}[\log \varphi_{\mu_k,1}(x_i)]\right)\,\\
...
\begin{example}
In a context of deep Gaussian models, $q_{\varphi}(\cdot|x)$ is a Gaussian probability density function with mean $\mu_{\varphi}(x)\in\rset^d$ and variance $\mathrm{diag}(\sigma_{\varphi}^2(x))\in\rset^{d\times d}$ where $(\mu_{\varphi}(x),\sigma_{\varphi}^2(x))$ is the output of a neural network with input $x$. Then, if $z = \mu_{\varphi}(x) + \mathrm{diag}(\sigma_{\varphi}(x))\varepsilon$ with $\varepsilon \sim \mathcal{N}(0,\Id_d)$, $z\sim q_{\varphi}(\cdot|x)$. Writing $g(\varepsilon, \varphi) = \mu_{\varphi}(x) + \sigma_{\varphi}(x)\varepsilon$, the Jacobian of $g$ with respect to $\varepsilon$ is
$$
J_\varepsilon[g](\varepsilon, \varphi) = \mathrm{diag}(\sigma_{\varphi}(x))\eqsp.
$$
...
\end{equation}
The \emph{log-likelihood function} is defined, droping the dependency on $(y_0, \dots, y_n)$, as
\begin{equation}\label{eq:log-likelihood-HMM}
\ell_{n} \;:=\; \log L_{n}.
\end{equation}
\end{definition}
The likelihood function given in Definition~\ref{def:likelihood-HMM} is not availbale in closed form. Therefore, in a setting where the law of the HMM depends on au unknown parameter $\theta\in\Theta$, and in the fully dominated case, the log-likelihood
$$
\ell_{n}(\theta) := \log
\int \nu_\theta(x_0) g_\theta(x_0, y_0) 
\prod_{k=1}^{n} Q_\theta(x_{k-1}, x_k) g_\theta(x_k, y_k) \mu^{\otimes (n+1)}(\rmd x_{0:n})
$$
cannot be optimized easily. A natural solution is to estimate $\theta$ using the EM algorithm. Therefore, at iteration $p\geq 0$ with a current parameter estimate $\widehat \theta_p$ it is required to compute:
\begin{align*}
Q(\theta,\widehat \theta_p) &= \mathbb{E}_{\widehat \theta_p}\left[\log p_\theta(X_{0:n},Y_{0:n})\middle| Y_{0:n}\right]\\
&=  \mathbb{E}_{\widehat \theta_p}\left[\log \nu_\theta(X_{0})g_\theta(X_0,Y_0)\middle| Y_{0:n}\right] + \sum_{k=1}^n \mathbb{E}_{\widehat \theta_p}\left[\log q_\theta(X_{k-1},X_k)g_\theta(X_k,Y_k)\middle| Y_{0:n}\right].
\end{align*}
This means that we need to sample (at least approximately) from the conditional distributions of $(X_{k-1},X_k)$ given $Y_{0:n}$ for a given parameter value $\widehat \theta_p$. This is in general not possible exactly and an appealing solution is to use Sequential Monte Carlo methods. In the next subsection we present these sequential methods, omitting the dependency on $\widehat \theta_p$ for simplicity.
...
\begin{itemize}
\item For all $0\leq k \leq n$, $\phi_k$ is the conditional distribution of $X_k$ given $Y_{0:k}$, and is referred to as the filtering distribution at time $k$.
\item For all $0\leq k \leq \ell \leq n$, $\phi_{k:\ell|n}$ is the conditional distribution of $X_{k:\ell}$ given $Y_{0:n}$, and is referred to as the joint smoothing  distribution of $X_{k:\ell}$ given $Y_{0:n}$.
\item  For all $0\leq k \leq n$, $\phi_{k|n} = \phi_{k:k|n}$, is the marginal smoothing distribution at time $k$.
\end{itemize}
...
{\int g_0(x_0)\, \nu(\mathrm{d}x_0)}.
\]
Therefore, $\phi_0(f)$ can be estimated using self-normalized importance sampling. Sample $(\xi_0^1,\ldots,\xi_0^N)$ i.i.d. with probability density $\chi$ and associate each $\xi_0^i$ with $\omega_0^i = g_0(\xi^i_0)/\chi(\xi_0^i)$ and compute
$$
\phi^N_0(f) = \sum_{i=1}^N\frac{\omega_0^i}{\sum_{j=1}^N\omega_0^j}f(\xi_0^i).
$$
In addition, for all $n\geq 0$ and all bounded measurable functions $f$,
$$
\phi_{0:n|n}(f) =  \frac{\int \nu(x_0) g_0(x_0) 
\prod_{k=1}^{n} q(x_{k-1}, x_k) g_k(x_k)f(x_{0:n})\mu^{\otimes (n+1)}(\mathrm{d}x_{0:n})}{\int \nu(x_0) g_0(x_0) 
\prod_{k=1}^{n} q(x_{k-1}, x_k) g_k(x_k)\mu^{\otimes (n+1)}(\mathrm{d}x_{0:n})}.
$$
Therefore, it is straightforward to note that for all $0\leq k \leq n-1$,
\begin{align*}
\phi_{0:k+1|k+1}(x_{0:k+1}) \propto \phi_{0:k|k}(x_{0:k})q(x_{k}, x_{k+1})g_{k+1}(x_{k+1})
\end{align*}
Therefore,  for all bounded measurable functions $f$,
$$
\phi_{k+1}(f) = \frac{\int \varphi_{k}(x_{k})q(x_{k}, x_{k+1})g_{k+1}(x_{k+1})f(x_{k+1})\mu(\rmd x_{k})\mu(\rmd x_{k+1})}{\int \phi_{k}(x_{k})q(x_{k}, x_{k+1})g_{k+1}(x_{k+1})\mu(\rmd x_{k})\mu(\rmd x_{k+1})}.
$$
...
\begin{align*}
\tilde\phi^N_{k+1}(x_{k+1}) \propto \sum_{i=1}^N\omega_k^iq(\xi^i_{k}, x_{k+1})g_{k+1}(x_{k+1}).
\end{align*}
...
$$
\phi^N_{k+1}(f) = \sum_{i=1}^N\frac{\omega_{k+1}^i}{\sum_{j=1}^N\omega_{k+1}^j}f(\xi_{k+1}^i).
$$
