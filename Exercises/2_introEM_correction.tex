...
\vspace{.2cm}
{\em
Pour tout $1\leq i \leq n$, $1\leq k \leq K$, par la formule de Bayes,
$$
\omega_{ik}^{(p)} = \mathbb{P}_\theta(Z_i=k\mid x_i,y_i) = \frac{\pi_k^{(p)}\,\varphi_{x_i^\top\beta^{(p)}_k,\sigma_k^{(p),2}}(y_i)}{\sum_{j=1}^K \pi_j^{(p)}\,\varphi_{x_i^\top\beta^{(p)}_k,\sigma_k^{(p),2}}(y_i)}.
$$
Ainsi, pour tout $\theta\in\Theta$,
$$
Q(\theta,\theta^{(p)}) = \sum_{i=1}^n\sum_{k=1}^K \omega_{ik}^{(p)}\left\{ \log \pi_k - \frac{1}{2} \log (2\pi \sigma_k^2) - \frac{1}{2\sigma_k^2}\left(y_i - x_i^\top \beta_k\right)^2\right\} \eqsp.
$$
}
