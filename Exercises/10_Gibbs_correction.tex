...
&\hspace{-5cm}= (n_k+\lambda_k)\left(m_k-\tau_k\right)^2 - \frac{\left(\sum_{i=1}^n\mathds{1}_{z_i=k}x_i+\lambda_k\alpha_k\right)^2}{n_k+\lambda_k} + \lambda_k\alpha_k^2 +\sum_{i=1}^n\mathds{1}_{z_i=k}x^2_i\,,
\end{align*}
...
o\`u 
$$
\rho_k = \beta_k+ \lambda_k\alpha_k^2 +\sum_{i=1}^n\mathds{1}_{z_i=k}x^2_i - \frac{\left(\sum_{i=1}^n\mathds{1}_{z_i=k}x_i+\lambda_k\alpha_k\right)^2}{n_k+\lambda_k}\,.
$$
Finalement, sous $\pi(m,s^2|x,z)$, les $s_k^2$ sont ind\'ependants de loi $\mathcal{IG}((n_k+\lambda_k)/2;\rho_k/2)$ et les $m_k$ sont ind\'ependants conditionnellement aux $s_k^2$ et de loi $\mathcal{N}(\tau_k;s_k^2/(n_k+\lambda_k))$, $1\leq k\leq K$.
}
